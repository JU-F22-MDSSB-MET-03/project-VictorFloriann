...
ggplot(climate_change, aes(year_date, Temp)) + 
  ...
  labs(title = "World has agreed - Temperature is rising",
       subtitle = "Global temperature rise from 1985 to 2022 (°C)",
       #x = "Year", 
       y = "Global Earth Temperature",
       color = "Legend",
       caption = "NASA: https://earthobservatory.nasa.gov/world-of-change/global-temperatures",) +
  scale_colour_manual(values = c("orange", "blue"),
                      #values = c("#5C1AAE", "#AE5C1A"),
...
ggplot(data=continent_data, aes(x=Entity, y=affected_all_disasters)) +
    ...
    coord_flip() +
    ylab("Number of People affected by climate disasters") +
    labs(title = 'Increase in people affected in ASIA by disasters caused by climate change',
         subtitle = "Climate related disaster"
       ) + 
    theme_minimal() 
    #guides(fill=FALSE)
...
ggplot(data=continent_data, aes(x=Entity, y=deaths_all_disasters)) +
    geom_bar(colour="black", fill="#DD8888", width=.8, stat="identity",position=position_dodge()) +
    coord_flip() +
    ylab("Number of People died by climate disasters") +
    labs(title = 'Increase in people death in ASIA and North America by disasters caused by climate change',
         subtitle = "Climate related disaster"
       ) + 
    theme_minimal() 
```
...
ggplot(data=continent_data, aes(x=Entity, y=injured_all_disasters)) +
    geom_bar(colour="black", fill="#DD8888", width=.8, stat="identity",position=position_dodge()) +
    coord_flip() +
    ylab("Number of People injured by climate disasters") +
    labs(title = 'Increase in people injured in ASIA by disasters caused by climate change',
         subtitle = "Climate related disaster"
       ) + 
    theme_minimal() 
```
...
disaster_data_country <- filter(disaster_data_country, (region %in% c("Asia", "Americas","Africa"))) %>% 
                        drop_na() 
